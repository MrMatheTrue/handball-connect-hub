...
const GROQ_API_KEY = "gsk_free"; // Usuário deve configurar sua chave Groq gratuita
...
export default function PlayerSearchWidget() {
  ...
  const [groqKey, setGroqKey] = useState(
    localStorage.getItem("hz_groq_key") || ""
  );
  ...
  const parseWithGroq = async (text: string): Promise<ParsedCriteria> => {
    const key = groqKey || localStorage.getItem("hz_groq_key");
    if (!key) return {};
    ...
    <div className="glass-card rounded-2xl p-6 border border-primary/20 mb-8">
      ...
      <div className="flex items-center justify-between mb-4">
        ...
        <button
          ...
          title="Configurar IA"
        >
          {groqKey ? "✅ IA ativa" : "⚙️ Configurar IA"}
        </button>
      </div>
      ...
      {showKeyInput && (
        <div className="mb-4 p-3 rounded-xl bg-secondary/50 border border-border space-y-2">
          <p className="text-xs text-muted-foreground">
            ...
            <a href="https://console.groq.com" target="_blank" className="text-primary underline">
              console.groq.com
            </a>{" "}
            e cole abaixo para ativar a IA:
          </p>
          <div className="flex gap-2">
            ...
            />
            <Button size="sm" onClick={saveGroqKey}>Salvar</Button>
